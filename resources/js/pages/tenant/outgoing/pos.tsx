...
import { Banknote, CreditCard, Minus, Plus, ShoppingCart, Trash2, X, Zap } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { cn } from '@/lib/utils';

function CartQtyInput({ quantity, available, onChange }: { quantity: number; available: number; onChange: (v: number) => void }) {
    ...
    return (
        <input
            ...
            className="w-12 border-0 bg-transparent text-center tabular-nums text-sm outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
        />
    );
}
...
type CartItem = { product_id: number; product_name: string; unit: string; currency: string; quantity: number; retail_price: number; available: number };
...
export default function OutgoingPos({ customers, warehouses, stock }: Props) {
    ...
    function addToCart(item: StockItem, qty = 1) {
        ...
        setCart((prev) => {
            ...
            if (idx !== -1) {
                const updated = [...prev];
                updated[idx] = { ...updated[idx], quantity: Math.min(updated[idx].quantity + qty, item.available) };
                return updated;
            }
            return [...prev, {
    ...
    function handleSearchEnter() {
        ...
        if (byBarcode) {
            ...
            const availableMatch = inStock.filter((s) =>
                s.product_name.toLowerCase().includes(q) ||
                (s.product_sku?.toLowerCase().includes(q) ?? false) ||
                (s.product_barcode?.toLowerCase().includes(q) ?? false),
            );
    ...
    function setQty(productId: number, qty: number) {
        ...
        setCart((prev) => prev.map((c) => c.product_id === productId ? { ...c, quantity: Math.min(qty, c.available) } : c));
    }
    ...
    function handlePay() {
        ...
        if (paymentMethod === 'cash' && hasReceivedInput && receivedNum < total) {
            setError(`Недостаточно: получено ${receivedNum.toLocaleString()}, нужно ${total.toLocaleString()} ${currency}`);
            return;
        }
        setSubmitting(true);
        router.post('/outgoing/pos', {
            date: new Date().toISOString().slice(0, 10),
            warehouse_id: warehouseId,
            customer_id: customerId || null,
            items: cart.map((i) => ({ product_id: i.product_id, quantity: i.quantity, retail_price: i.retail_price })),
        }, {
            onSuccess: () => { setCart([]); setQuery(''); setCustomerId(''); setReceived(''); setSubmitting(false); focusSearch(); },
            onError: (errs) => { setError(Object.values(errs)[0] as string ?? 'Ошибка'); setSubmitting(false); },
        });
    }

    function handleClear() { setCart([]); setError(''); setReceived(''); focusSearch(); }
                ...
                <div className="flex flex-1 flex-col overflow-hidden border-r">
                    <div className="flex items-center gap-2 border-b px-4 py-3">
                        ...
                        <button
                            ...
                            title={autoAdd ? 'Авто-добавление включено' : 'Авто-добавление выключено'}
                            className={cn(
                    ...
                    <div className="grid flex-1 auto-rows-max grid-cols-2 gap-2 overflow-y-auto p-4 sm:grid-cols-3 lg:grid-cols-4">
                        {displayItems.map((item) => {
                            const inCart = cart.find((c) => c.product_id === item.product_id)?.quantity ?? 0;
                            const outOfStockItem = item.available <= 0;
                            const exhausted = outOfStockItem || inCart >= item.available;
                            return (
                                <button
                                    key={item.product_id}
                                    onClick={() => { if (!outOfStockItem) { addToCart(item); focusSearch(); } }}
                                    disabled={exhausted}
                                    className={cn(
                                ...
                                >
                                    ...
                                    <span className="mt-auto pt-2 font-semibold tabular-nums">
                                        {Number(item.retail_price).toLocaleString()} {item.currency}
                                    </span>
                                    {outOfStockItem ? (
                                        <span className="text-xs font-medium text-destructive">Нет в наличии</span>
                                    ) : (
                                        <span className="text-xs text-muted-foreground">
                                            {inCart > 0 ? `${inCart} / ${item.available}` : item.available} {item.unit}
                                        </span>
                                    )}
                                </button>
                            );
                ...
                <div className="flex w-80 flex-col lg:w-96">
                                ...
                                {cart.map((item) => (
                                    <div key={item.product_id} className="flex items-center gap-2 px-4 py-2.5">
                                        ...
                                        <div className="flex items-center gap-1">
                                            ...
                                            <CartQtyInput quantity={item.quantity} available={item.available} onChange={(v) => setQty(item.product_id, v)} />
                                            <Button variant="ghost" size="icon" className="size-7" onClick={() => setQty(item.product_id, item.quantity + 1)} disabled={item.quantity >= item.available}>
                    ...
                    <div className="flex flex-col gap-3 border-t p-4">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <button
                                ...
                                className={cn(
                                    ...
                                    paymentMethod === 'cash' ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:bg-muted',
                                )}
                            >
                                <Banknote className="size-4" /> Наличные
                            </button>
                            <button
                                onClick={() => { setPaymentMethod('card'); setReceived(''); }}
                                className={cn(
                                    'flex items-center justify-center gap-1.5 rounded-lg border py-2 text-sm font-medium transition-colors',
                                    paymentMethod === 'card' ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:bg-muted',
                                )}
                            >
                        ...
                        {paymentMethod === 'cash' && (
                            <>
                                ...
                                <div className="flex items-center gap-2">
                                    <span className="shrink-0 text-sm text-muted-foreground">Получено</span>
                                    <input
                                        type="number"
                                        value={received}
                                        onChange={(e) => { setReceived(e.target.value); setError(''); }}
                                        onFocus={(e) => e.target.select()}
                                        placeholder={total > 0 ? total.toLocaleString() : '0'}
                                        className="min-w-0 flex-1 rounded-lg border px-3 py-1.5 text-right text-base font-semibold tabular-nums outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                                    />
                                </div>
                                ...
                                <div className="flex flex-wrap gap-1">
                                    {QUICK_AMOUNTS.map((amt) => (
                                        <button
                                            key={amt}
                                            onClick={() => setReceived(String((parseFloat(received) || 0) + amt))}
                                ...
                                {hasReceivedInput && (
                                    <div className={cn(
                                        ...
                                        change < 0 ? 'bg-destructive/10' : 'bg-green-50',
                                    )}>
                                        <span className="text-sm font-medium">Сдача</span>
                                        <span className={cn('text-lg font-bold tabular-nums', change < 0 ? 'text-destructive' : 'text-green-700')}>
                        ...
                        <Button className="w-full" size="lg" onClick={handlePay} disabled={submitting || cart.length === 0}>
                            {submitting ? 'Сохраняем...' : paymentMethod === 'card' ? 'Оплатить картой (F2)' : 'Принять оплату (F2)'}
                        </Button>
                        <Button variant="outline" className="w-full" onClick={handleClear} disabled={cart.length === 0}>
                            Очистить (Esc)
                        </Button>
                    </div>
                </div>
